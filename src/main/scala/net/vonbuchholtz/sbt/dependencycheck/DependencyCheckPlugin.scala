...
object DependencyCheckPlugin extends sbt.AutoPlugin {
  ...
  def failBuildOnCVSS(dependencies: Array[Dependency], cvssScore: Float): Boolean = dependencies.exists(p => {
    p.getVulnerabilities.asScala.exists(v => {
      (v.getCvssV2 != null && v.getCvssV2.getImpactScore >= cvssScore) || (v.getCvssV3 != null && v.getCvssV3.getImpactScore >= cvssScore || (v.getUnscoredSeverity != null && SeverityUtil.estimateCvssV2(v.getUnscoredSeverity) >= cvssScore)) || (cvssScore <= 0.0f)
    })
  })
